...
case "${ID:-}" in ubuntu|debian) ok "${PRETTY_NAME:-Debian/Ubuntu}" ;; *) die "Supported OS: Debian/Ubuntu." ;; esac
...
apt-get update -y
apt-get install -y python3 python3-pip python3-venv python3-dev sqlite3 curl ca-certificates iproute2 openssl
...
"$PYTHON_BIN" -m venv --help >/dev/null || die "Python venv is unavailable."
ok "Python $PYVER ready."

log "Selecting an available HTTP port..."
while ss -H -lnt 2>/dev/null | awk '{print $4}' | grep -Eq "(:|\])${PORT}$"; do
...
[[ -f "$SOURCE_DIR/requirements.txt" && -f "$SOURCE_DIR/app/main.py" ]] || die "Run install.sh from the project directory."
...
"$APP_DIR/.venv/bin/python" -m pip install --disable-pip-version-check --no-cache-dir --upgrade pip wheel
"$APP_DIR/.venv/bin/python" -m pip install --disable-pip-version-check --no-cache-dir -r "$APP_DIR/requirements.txt"

read -rp "Admin username [admin]: " ADMIN_USER
ADMIN_USER="${ADMIN_USER:-admin}"
[[ "$ADMIN_USER" =~ ^[A-Za-z0-9_.-]{3,32}$ ]] || die "Username must be 3-32 characters: A-Z, a-z, 0-9, _, ., -."

while true; do
  read -rsp "Admin password (minimum 12 characters): " ADMIN_PASS; echo
  (( ${#ADMIN_PASS} >= 12 )) || { warn "Password is too short."; continue; }
...
for n in (16384,8192,4096):
    try:
        digest=hashlib.scrypt(password, salt=salt, n=n, r=8, p=1, dklen=64)
...
""")
con.execute("INSERT OR REPLACE INTO settings(key,value) VALUES('username',?)",(user,))
con.execute("INSERT OR REPLACE INTO settings(key,value) VALUES('password',?)",(ph,))
