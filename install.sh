...
# If install.sh is directly inside the project.
if [[ \
    ! -f "$SOURCE_DIR/requirements.txt" ||
    ! -f "$SOURCE_DIR/app/main.py"
 ]]; then

    # Check nested v2 project:
    #
    # /opt/idontScanner/install.sh
    # /opt/idontScanner/idontScanner-2.0.0/

    if [[ \
        -f "$SOURCE_DIR/idontScanner-2.0.0/requirements.txt" &&
        -f "$SOURCE_DIR/idontScanner-2.0.0/app/main.py"
    ]]; then

        SOURCE_DIR="$SOURCE_DIR/idontScanner-2.0.0"
...
# When launched remotely with:
#
# curl -fsSL .../install.sh | sudo bash
#
# the script itself is not located inside the project tree.
# In that case download the matching repository archive.
...
if [[ \
    ...
    # Keep temporary files cleanup separate from the main
    # installation error trap.
...
# IMPORTANT:
# When the installer is executed with:
#
# curl ... | sudo bash
#
# stdin belongs to curl.
#
# Therefore all interactive input is explicitly read from /dev/tty.

if [[ ! -r /dev/tty ]]; then
    die "Interactive terminal is required for setting admin credentials."
fi
...
while true; do
    ...
    read -r -s \
        -p "Admin password (minimum 12 characters): " \
        ADMIN_PASS < /dev/tty
...
# The app uses the same scrypt format and has its own
# low-memory fallback.
...
for n in (16384, 8192, 4096):
    ...
    raise SystemExit(
        f"Unable to hash password: {last}"
    )
