...
apt-get update -y
apt-get install -y python3 python3-pip python3-venv python3-dev curl ca-certificates openssl unzip iproute2 rsync
...
# Give Uvicorn enough time to initialize on slower VPS instances.
HEALTH_OK=0
for _ in $(seq 1 30); do
  if systemctl is-active --quiet "$SERVICE_NAME" && \
     curl -fsS --connect-timeout 1 --max-time 3 "http://127.0.0.1:${PORT}/login/" >/dev/null 2>&1; then
    HEALTH_OK=1
    break
  fi
  sleep 1
...
if [[ "$HEALTH_OK" -ne 1 ]]; then
  ...
  die "HTTP health check failed on 127.0.0.1:${PORT}. The service did not become ready within 30 seconds."
fi
