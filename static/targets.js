(() => {
 ...
 document.querySelectorAll('.custom-select').forEach(root=>{
   ...
   m?.querySelectorAll('[data-value]').forEach(o=>o.addEventListener('click',()=>{
     ...
     t.querySelector('span').textContent=o.querySelector(':scope > span:last-child')?.textContent.trim() || o.textContent.trim();
 ...
 function statusPill(ok,label){return `<span class="target-status ${ok?'good':'bad'}">${icon(ok?'check':'close')}<span>${esc(label)}</span></span>`;}
